...
任务工具（task tool）会启动第二个智能体循环，并为其创建一个全新的消息列表。
两个循环共享同一个工作目录，但只有子智能体最终生成的文本会返回给父级对话。
...
子智能体不包含任务工具（task tool），因此它无法再次进行任务委托。
"""

# 注意该节主要用于教学subagent，因此没有上一节的todo_write
